...
def create_result_plot(
    result_file_names, result_figure_prefix="test_perf_result"
):
    ...
    try:
        ...
        for i, name in enumerate(result_file_names):
            ...
            for axes_obj, metrics_name in zip(axes_indexs, metrics_names):
                axes_obj.plot(
                    x, df[metrics_name], linewidth=2, color=color, label=name
                )
        ...
        if len(result_file_names) == 1:
            plt.savefig(
                f"./{result_file_names[0]}.png", dpi=200, bbox_inches="tight"
            )
            print(f"Result figure is locate in {result_file_names[0]}.png")
        else:
            today = date.today()
            plt.savefig(
                f"./{result_figure_prefix}_{today}.png",
                dpi=200,
                bbox_inches="tight",
            )
            print(
...
def create_ttft_plot(
    ...
    color_map = {
        name: colors[i % len(colors)] for i, name in enumerate(metrics_names)
    }
    ...
    try:
        ...
        for file_name in result_file_names:
            ...
            file_data["pd_first_token_mean"] = file_data[
                pd_first_token_columns
            ].mean(axis=1)

            file_data["pd_decode_mean"] = (
                file_data["pd_first_token_mean"]
                - file_data["pd_prefill_mean"]
                - file_data["pd_queue_mean"]
            )
            ...
            file_data["others"] = (
                file_data["ttft_mean"]
                - file_data["e_prefill_mean"]
                - file_data["e_queue_mean"]
                - file_data["transfer_to_encode"]
                - file_data["transfer_to_pd"]
                - file_data["pd_first_token_mean"]
            )
        ...
        for idx_idx, index_val in enumerate(unique_indices):
            ...
            for metrics_name in metrics_names:
                ...
                # Add value labels
                for value, bar, single_bottom in zip(
                    index_data[metrics_name], bars, bottom
                ):
                    if value > 0:  # Only display values greater than 0
                        ax.text(
                            bar.get_x() + bar.get_width() / 2,
                            single_bottom
                            + value / 2,  # Display in the middle of the bar
                            f"{value:.1f}",
                            ha="center",
                            va="center",
                            fontsize=8,
            ...
            # Set x-axis labels
            file_labels = [
                f"{row['source_file']}" for _, row in index_data.iterrows()
            ]
            ax.set_xticks(x_pos)
            ...
            ax.set_title(
                f"TTFT Breakdown - Req Rate/Card: {index_val}",
                fontsize=14,
                fontweight="bold",
            )
        ...
        plt.tight_layout(
            rect=[0, 0, 1, 0.96]
        )  # Reserve space for the main title

        # Save the figure
        if len(result_file_names) == 1:
            plt.savefig(
                f"./{result_file_names[0]}.png",
                dpi=200,
                pad_inches=0.1,
                bbox_inches="tight",
            )
            ...
            print(
                f"Result figure is located in {result_figure_prefix}_{today}.png"
            )
...
REQUEST_CONF_DIR = os.path.join(
    benchmark_path, "ais_bench/benchmark/configs/models/vllm_api"
)
DATASET_DIR = os.path.join(benchmark_path, "ais_bench/datasets")
CONSTS_DIR = os.path.join(
    benchmark_path, "ais_bench/benchmark/global_consts.py"
)
...
class AisbenchRunner:
    ...
    def _performance_result_save(self):
        try:
            ...
            merged_json["Total Token Throughput/Card"] = round(
                float(
                    merged_json.get("Total Token Throughput")
                    .get("total")
                    .split(" ")[0]
                )
                / self.card_num,
            ...
            merged_json["Request Throughput/Card"] = round(
                float(
                    merged_json.get("Request Throughput")
                    .get("total")
                    .split(" ")[0]
                )
                / self.card_num,
    ...
    def _init_dataset_conf(self):
        ...
        if self.dataset_conf.startswith("textvqa"):
            self.dataset_path = os.path.join(
                self.dataset_path, "textvqa_val.jsonl"
            )
    ...
    def _init_consts_conf(self):
        with open(CONSTS_DIR, "r", encoding="utf-8") as f:
            content = f.read()
        content = re.sub(
            r"PRESSURE_TIME.*", f"PRESSURE_TIME = {self.pressure_time}", content
        )
    ...
    def _init_request_conf(self):
        ...
        content = re.sub(
            r"batch_size.*", f"batch_size = {self.batch_size},", content
        )
        ...
        conf_path_new = os.path.join(
            REQUEST_CONF_DIR, f"{self.request_conf}_custom.py"
        )
        with open(conf_path_new, "w", encoding="utf-8") as f:
            f.write(content)
    ...
    def _wait_for_exp_folder(self):
        while True:
            ...
            if "Current exp folder: " in line:
                self.exp_folder = re.search(
                    r"Current exp folder: (.*)", line
                ).group(1)
                return
            if "ERROR" in line:
                raise RuntimeError(
                    "Some errors happen to Aisbench task."
                ) from None
    ...
    def _get_result_performance(self):
        ...
        result_json_file = os.path.join(
            result_dir, f"{dataset_type}dataset.json"
        )
    ...
    def _performance_verify(self):
        ...
        success_num = self.result_json["Success Requests"]["total"]
        total_num = self.result_json["Total Requests"]["total"]
        output_success_num = self.result_csv.loc[
            self.result_csv["Performance Parameters"] == "TPOT", "N"
        ].values[0]
        ...
        assert int(success_num) == total_num, (
            f"Performance verification failed. The current Success Request is {success_num}, which is not equal to {total_num}."
        )
        assert int(output_success_num) == total_num, (
            f"Performance verification failed. {total_num - output_success_num} Request output is empty."
        )
...
def run_aisbench_cases(
    model, port, aisbench_cases, card_num=1, verify=True, save=True
):
    aisbench_errors = []
    ...
    assert not aisbench_errors, (
        "some aisbench cases failed, info were shown above."
    )
